...
impl<const N : usize> CompPath<N> 
{
    ...
    pub fn corr_node(&mut self, comps : &[Box<dyn Component>; N], compl : &[[[f32; 2]; 3]; N], index : usize, o_index : usize, f_corr : f32) -> bool {
        ...
        if f_s < 1.0 {
            self.backwards_corr(comps, scale_array(self.omegas[index], f_s * f_corr), index, f_corr);
    ...
    pub fn backwards_corr(&mut self, comps : &[Box<dyn Component>; N], omegas : [f32; N], mut index : usize, f_corr : f32) {
        ...
        loop {
            ...
            if !self.corr_node(comps, &compl, index, index - 1, f_corr) {
                break;
            }
    ...
    pub fn generate(&mut self, comps : &[Box<dyn Component>; N], vel_0 : [f32; N], vel_end : [f32; N], f_corr : f32) {
        ...
        for i in 0 .. (path_len - 1) {
            ...
            self.corr_node(comps, &compl, i, i + 1, f_corr);
        }
        
        self.backwards_corr(comps, vel_end, path_len - 1, f_corr);
    }
